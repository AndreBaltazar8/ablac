...
printf -v runtime_compile_command \
    'clang -std=c11 -O2 -ffunction-sections -fdata-sections -I%q -c %q -o %q && clang -std=c11 -O2 -ffunction-sections -fdata-sections -I%q -c %q -o %q' \
...
printf -v link_command \
    'llvm_dir=$(llvm-config --libdir); clang -fuse-ld=lld -Wl,--threads=1 -Wl,--gc-sections -Wl,--export-dynamic %q %q %q -L"$llvm_dir" -Wl,-rpath,"$llvm_dir" -lLLVM -o %q' \
