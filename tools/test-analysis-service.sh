...
responses=$(
    printf '%s\n' \
        ...
        '{"schema":1,"id":3,"method":"document/open","params":{"uri":"file:///tmp/abla-analysis-caller.ab","version":2,"text":"import \"abla-analysis-overlay.ab\"\nval callback = greet\nfun main(input: string): string {\n    val message = greet(input)\n    message\n}\n"}}' \
        '{"schema":1,"id":4,"method":"document/open","params":{"uri":"file:///tmp/abla-analysis-broken.ab","version":1,"text":"fun broken: int = missing\n"}}' \
        '{"schema":1,"id":5,"method":"document/open","params":{"uri":"file:///tmp/independent-example.ab","version":1,"text":"fun greet: int = 42\n"}}' \
        '{"schema":1,"id":6,"method":"analyze","params":{}}' \
        '{"schema":1,"id":7,"method":"document/change","params":{"uri":"file:///tmp/abla-analysis-overlay.ab","version":6,"text":"fun stale: int = 0\n"}}' \
        '{"schema":1,"id":8,"method":"refactor/validate","params":{"baseRevision":"5","edits":[{"uri":"file:///tmp/abla-analysis-broken.ab","start":18,"end":25,"newText":"1"}],"invariants":["no-new-errors"]}}' \
        '{"schema":1,"id":9,"method":"refactor/validate","params":{"baseRevision":"5","edits":[{"uri":"file:///tmp/abla-analysis-caller.ab","start":49,"end":54,"newText":"unknownName"}],"invariants":["no-new-errors"]}}' \
        '{"schema":1,"id":10,"method":"refactor/validate","params":{"baseRevision":"5","edits":[{"uri":"file:///tmp/abla-analysis-broken.ab","start":18,"end":25,"newText":"otherMissing"}],"invariants":["no-new-errors"]}}' \
        '{"schema":1,"id":11,"method":"shutdown","params":{}}' |
        "$compiler" analyze --stdio
)

node -e '
const lines = require("node:fs").readFileSync(0, "utf8").trim().split("\n").map(JSON.parse);
if (lines.length !== 11) throw new Error(`expected 11 responses, got ${lines.length}`);
...
const document = lines[5].result.documents[0];
...
const caller = lines[5].result.documents[1];
...
const semantic = lines[5].result.documents[2].diagnostics[0];
...
const independent = lines[5].result.documents[3];
if (independent.symbols[0]?.name !== "greet" || independent.diagnostics.length !== 0) {
  throw new Error("an unrelated duplicate declaration contaminated another semantic component");
}
if (lines[6].error?.code !== "stale_document") {
  throw new Error("stale overlay version was accepted");
}
if (lines[7].result?.valid !== true || !lines[7].result?.snapshot) {
  throw new Error("a diagnostic-removing prospective edit was rejected");
}
if (lines[8].result?.valid !== false) {
  throw new Error("a diagnostic-introducing prospective edit was accepted");
}
if (lines[9].result?.valid !== false) {
