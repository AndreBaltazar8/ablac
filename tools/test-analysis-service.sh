...
responses=$(
    printf '%s\n' \
        ...
        '{"schema":1,"id":3,"method":"analyze","params":{}}' \
        '{"schema":1,"id":4,"method":"document/change","params":{"uri":"file:///tmp/abla-analysis-overlay.ab","version":6,"text":"fun stale: int = 0\n"}}' \
        '{"schema":1,"id":5,"method":"shutdown","params":{}}' |
        "$compiler" analyze --stdio
)

node -e '
const lines = require("node:fs").readFileSync(0, "utf8").trim().split("\n").map(JSON.parse);
if (lines.length !== 5) throw new Error(`expected 5 responses, got ${lines.length}`);
...
const document = lines[2].result.documents[0];
...
if (lines[3].error?.code !== "stale_document") {
  throw new Error("stale overlay version was accepted");
}
' <<<"$responses"

printf '%s\n' 'analysis service: protocol, overlay, UTF-8 spans, stale-version rejection passed'
