...
responses=$(
    printf '%s\n' \
        ...
        '{"schema":1,"id":6,"method":"document/open","params":{"uri":"file:///tmp/malformed.ab","version":1,"text":"fun broken(: int = 1\n"}}' \
        '{"schema":1,"id":7,"method":"analyze","params":{}}' \
        '{"schema":1,"id":8,"method":"document/change","params":{"uri":"file:///tmp/abla-analysis-overlay.ab","version":6,"text":"fun stale: int = 0\n"}}' \
...
const document = lines[6].result.documents[0];
...
const caller = lines[6].result.documents[1];
...
const semantic = lines[6].result.documents[2].diagnostics[0];
...
const independent = lines[6].result.documents[3];
...
const parserDiagnostics = lines[6].result.documents[4].diagnostics;
if (parserDiagnostics.length !== 4 || parserDiagnostics[0]?.range.start !== 11) {
  throw new Error("parser diagnostics did not preserve individual source ranges");
}
if (lines[7].error?.code !== "stale_document") {
  throw new Error("stale overlay version was accepted");
}
if (lines[8].result?.valid !== true || !lines[8].result?.snapshot) {
