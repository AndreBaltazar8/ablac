...
if [[ $c_status -ne 42 ]]; then
    echo "borrow lifetimes: generated C expected 42, got $c_status" >&2
    exit 1
fi
...
echo 'borrow lifetimes: direct/indirect/global/returned callable contracts + stored/chained views + CFG/staged verification + LLVM/C passed'
